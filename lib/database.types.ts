...
export type Database = {
  public: {
    Tables: {
      ...
      session_logs: Table<{ id:string; member_id:string; staff_user_id:string|null; trained_at:string; session_label:string|null; notes:string|null; sessions_left_after:number; created_at:string; booking_id:string|null; member_package_id:string|null }>
      package_definitions: Table<{ id:string; code:string; name:string; service_category:string; included_sessions:number; validity_days:number|null; shareable:boolean; billing_mode:string; active:boolean; standard_price:number|null; created_at:string; updated_at:string }>
      ...
      availability_rules: Table<{ id:string; coach_user_id:string; branch:string; session_type:string; session_label:string|null; weekday:number; local_start_time:string; local_end_time:string; slot_duration_minutes:number; capacity:number; valid_from:string; valid_until:string|null; active:boolean; created_by:string; created_at:string; updated_at:string }>
      schedule_slots: Table<{ id:string; availability_rule_id:string|null; coach_user_id:string|null; branch:string; session_type:string; session_label:string|null; start_at:string; end_at:string; capacity:number; status:string; created_by:string; created_at:string; updated_at:string }>
      bookings: Table<{ id:string; member_id:string; slot_id:string|null; request_kind:string; status:string; requested_coach_user_id:string|null; assigned_coach_user_id:string|null; branch:string; session_type:string; session_label:string|null; requested_start_at:string; requested_duration_minutes:number; start_at:string|null; end_at:string|null; member_package_id:string|null; cancelled_at:string|null; cancel_reason:string|null; no_show_charged:boolean; created_by:string; created_at:string; updated_at:string }>
    ...
    Functions: {
      ...
      member_bearforce_history: { Args:{p_limit?:number}; Returns:Json }
      member_bearforce_seasons: { Args:never; Returns:Json }
      ...
      staff_package_catalog: { Args:never; Returns:Json }
      admin_update_package_settings: { Args:{p_package_id:string;p_standard_price:number|null;p_active:boolean}; Returns:Json }
      member_request_slot: { Args:{p_slot_id:string}; Returns:Json }
      member_request_custom_session: { Args:{p_session_label:string;p_requested_start_at:string;p_requested_coach_user_id?:string|null;p_duration_minutes?:number}; Returns:Json }
      member_cancel_booking: { Args:{p_booking_id:string;p_reason?:string|null}; Returns:Json }
      staff_create_availability_rule: { Args:{p_coach_user_id:string;p_branch:string;p_session_label:string;p_weekday:number;p_local_start_time:string;p_local_end_time:string;p_slot_duration_minutes?:number;p_capacity?:number;p_valid_from?:string;p_valid_until?:string|null}; Returns:string }
      staff_generate_slots: { Args:{p_rule_id:string;p_through:string}; Returns:number }
      staff_create_one_off_slot: { Args:{p_coach_user_id:string|null;p_branch:string;p_session_label:string;p_start_at:string;p_duration_minutes?:number;p_capacity?:number}; Returns:string }
      ...
      staff_qr_checkin: { Args:{p_member_code:string;p_notes?:string|null;p_booking_id?:string|null;p_member_package_id?:string|null;p_session_label?:string|null}; Returns:Json }

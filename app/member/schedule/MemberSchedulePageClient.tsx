  ...
  return <main className="min-h-screen bg-[#020b1c] text-white"><div className="mx-auto max-w-7xl p-4 md:p-8">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <section className="space-y-6"><div className="flex items-center justify-between"><div><h2 className="text-2xl font-bold">Available slots</h2><p className="text-sm text-white/50">Only your assigned branch is shown.</p></div><button onClick={()=>setCustomOpen(v=>!v)} className="rounded-2xl bg-[#ff7a1a] px-4 py-3 text-sm font-bold">Custom request</button></div>
        {customOpen&&<form onSubmit={requestCustom} className="rounded-[24px] border border-orange-400/30 bg-[#141414] p-5"><h3 className="font-bold">Custom request</h3><p className="mt-1 text-xs text-white/50">Choose Any available coach by leaving coach blank. Staff/admin must confirm the request.</p><select value={custom.sessionLabel} onChange={e=>setCustom({...custom,sessionLabel:e.target.value})} className="mt-4 w-full rounded-xl bg-[#242424] p-3">{SESSION_TAXONOMY.map(item=><option key={item.label} value={item.label}>{item.label}</option>)}</select><input type="datetime-local" value={custom.start} onChange={e=>setCustom({...custom,start:e.target.value})} className="mt-3 w-full rounded-xl bg-[#242424] p-3"/><select value={custom.coach} onChange={e=>setCustom({...custom,coach:e.target.value})} className="mt-3 w-full rounded-xl bg-[#242424] p-3"><option value="">Any available coach</option>{initialData.coaches.map(coach=><option key={coach.id} value={coach.id}>{coach.full_name}</option>)}</select><select value={custom.duration} onChange={e=>setCustom({...custom,duration:e.target.value})} className="mt-3 w-full rounded-xl bg-[#242424] p-3"><option value="60">60 minutes</option><option value="45">45 minutes</option><option value="30">30 minutes</option><option value="90">90 minutes</option></select><button disabled={working==="custom"} className="mt-4 rounded-xl bg-[#ff7a1a] px-5 py-3 font-bold">{working==="custom"?"Sending…":"Request custom session"}</button></form>}
        {grouped.length===0?<div className="rounded-[24px] border border-white/10 bg-[#141414] p-8 text-center text-white/50">No open slots yet. You can send a Custom request.</div>:grouped.map(([day,daySlots])=><div key={day}><h3 className="mb-3 text-lg font-bold">{day}</h3><div className="grid gap-3 md:grid-cols-2">{daySlots.map(slot=>{const e=initialData.eligibility[slot.session_type];const late=hoursAway(slot.start_at)<24;const disabled=late||!e?.can_request_booking||working===slot.id;return <article key={slot.id} className="rounded-[24px] border border-white/10 bg-[#141414] p-5"><div className="flex items-start justify-between"><div><div className="font-bold">{displaySessionLabel(slot.session_label,slot.session_type)}</div><div className="mt-2 flex items-center gap-2 text-sm text-white/60"><CalendarDays size={15}/>{manilaDate(slot.start_at)}</div><div className="mt-1 flex items-center gap-2 text-sm text-white/60"><Clock3 size={15}/>{Math.round((new Date(slot.end_at).getTime()-new Date(slot.start_at).getTime())/60000)} minutes</div></div><span className="rounded-full bg-white/5 px-3 py-1 text-xs">Capacity {slot.capacity}</span></div><p className="mt-3 text-xs text-white/45">Coach: {slot.coach_user_id ? (initialData.coaches.find(coach=>coach.id===slot.coach_user_id)?.full_name ?? "Specific coach") : "Any available coach"}</p>{late&&<p className="mt-3 text-xs text-amber-300">Inside 24 hours — contact staff/admin.</p>}{e?.warning_message&&<p className="mt-3 text-xs text-amber-300">{e.warning_message}</p>}{e?.blocking_reason&&<p className="mt-3 text-xs text-red-300">{e.blocking_reason}</p>}<button disabled={disabled} onClick={()=>void requestSlot(slot.id)} className="mt-4 w-full rounded-xl bg-[#ff7a1a] px-4 py-3 text-sm font-bold disabled:bg-white/10 disabled:text-white/30">{working===slot.id?"Requesting…":"Request"}</button></article>})}</div></div>)}
      </section>
      <aside><div className="rounded-[24px] border border-white/10 bg-[#141414] p-5"><h2 className="text-xl font-bold">My requests</h2><p className="mt-1 text-xs text-white/50">Confirmed sessions may be changed by you until 4 hours before start.</p><div className="mt-4 space-y-3">{bookings.length===0?<p className="text-sm text-white/45">No upcoming requests.</p>:bookings.map(b=><div key={b.id} className="rounded-2xl bg-white/5 p-4"><div className="flex items-center justify-between"><b>{displaySessionLabel(b.session_label,b.session_type)}</b><span className="text-xs uppercase text-orange-300">{b.status}</span></div><p className="mt-2 text-sm text-white/55">{manilaDate(b.start_at??b.requested_start_at)}</p><button disabled={working===b.id} onClick={()=>void cancelBooking(b)} className="mt-3 text-xs font-bold text-red-300">Cancel / change</button></div>)}</div></div></aside>
    </div>
  </div></main>
}

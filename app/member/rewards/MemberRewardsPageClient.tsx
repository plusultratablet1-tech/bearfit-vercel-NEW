...
import { Coins, Gift, PackageCheck, WalletCards } from "lucide-react"
...
const statusLabels: Record<RewardRequestItem["status"], string> = {
  pending: "Pending",
  approved: "Approved",
  claimed: "Claimed",
  rejected: "Rejected",
...
export default function MemberRewardsPageClient({
  initialSnapshot,
  initialError,
}: {
  initialSnapshot: MemberRewardsSnapshot | null
  ...
  const pendingRewardIds = useMemo(
    () => new Set(requests.filter((request) => request.status === "pending").map((request) => request.reward_id)),
    [requests],
  )
      ...
      <div className="mx-auto max-w-7xl px-4 py-5 pb-28 md:px-6 lg:px-8 lg:py-8 lg:pb-8">
        <header className="border-b border-white/10 pb-5">
          ...
          <p className="mt-1 text-sm text-white/45">
            Spend this season&apos;s available Bearforce Points without reducing your lifetime achievement.
          </p>
        </header>
        ...
        <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <BalanceCard
            ...
            sublabel={`Season ends ${seasonEndsAt ? dateTime(seasonEndsAt) : "—"}`}
            highlight
          />
        </section>
        ...
        <section className="mt-8">
          <div className="mb-4 flex items-end justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[.18em] text-white/35">Catalog</p>
              <h2 className="mt-1 text-2xl font-black">Choose a reward</h2>
            </div>
            <span className="text-xs text-white/35">{catalog.length} available</span>
          </div>

          {catalog.length === 0 ? (
            <div className="rounded-[24px] border border-dashed border-white/10 bg-[#141414] px-5 py-12 text-center">
              <Gift className="mx-auto text-[#ff7a1a]" />
              <h3 className="mt-3 text-xl font-bold">Rewards are being prepared</h3>
              <p className="mt-2 text-sm text-white/45">BearFit staff will publish real rewards here.</p>
            </div>
          ) : (
        ...
        <section className="mt-9 overflow-hidden rounded-[24px] border border-white/[0.06] bg-[#151515]">
          <div className="border-b border-white/[0.07] p-5">
            ...
            <h2 className="mt-1 text-2xl font-black">Your reward requests</h2>
          </div>
          <div className="p-4 md:p-5">
            {requests.length === 0 ? (
              <p className="py-8 text-center text-sm text-white/40">You haven&apos;t requested a reward yet.</p>
            ) : (
              <div className="space-y-3">
                {requests.map((request) => (
                  <div
                    key={request.id}
                    className="grid gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.025] p-4 md:grid-cols-[1fr_auto] md:items-center"
                  >
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="font-bold">{request.reward_title}</h3>
                        <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold capitalize ${statusStyle(request.status)}`}>
                          {statusLabels[request.status]}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-white/40">
                        {request.reward_category} · {points(request.points_cost)} points · {dateTime(request.requested_at)}
                      </p>
                      {request.decision_note && <p className="mt-2 text-xs text-white/55">{request.decision_note}</p>}
                    </div>
                    <div className="flex items-center gap-2">
                      {request.status === "pending" && (
                        <button
                          ...
                          className="min-h-11 rounded-xl border border-white/10 px-4 text-sm font-semibold text-white/70 disabled:opacity-50"
                        >
                          {working === request.id ? "Cancelling…" : "Cancel request"}
                        </button>
                      )}
                    </div>
                  </div>
...
function RewardCard({
  ...
  const reason = pending
    ...
    : outOfStock
      ? "Out of stock"
      : insufficient
        ? `${points(deficit)} more points needed`
        : membershipBlocked
          ? "Active membership required"
          : null

  return (
    <article className="overflow-hidden rounded-[24px] border border-white/[0.06] bg-[#171717]">
      <div className="relative h-40 bg-gradient-to-br from-[#ff7a1a] to-[#5f2706]">
        {reward.image_url ? (
        ...
        <span className="absolute bottom-3 left-3 rounded-full bg-black/55 px-3 py-1 text-[10px] font-bold uppercase tracking-[.15em]">
          {reward.category}
        </span>
      </div>
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-white/50">{reward.description || "BearFit member reward"}</p>
          </div>
          <span className="shrink-0 text-lg font-black text-[#ff9b54]">{points(reward.points_cost)} pts</span>
        </div>
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-white/45">
          {reward.stock_quantity === null ? <span>Unlimited</span> : <span>{reward.available_stock ?? 0} left</span>}
          {reward.requires_active_membership && <span>· Active membership</span>}
        </div>
        <button
          disabled={disabled}
          onClick={onRedeem}
          className="mt-5 min-h-12 w-full rounded-xl bg-[#ff7a1a] px-4 text-sm font-extrabold disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/35"
        >
          {working ? "Submitting…" : reason ?? "Redeem"}
        </button>
      </div>
    </article>
  )
...
function BalanceCard({
  ...
  return (
    <div className={`rounded-[22px] border p-5 ${highlight ? "border-[#ff7a1a]/30 bg-[#ff7a1a]/10" : "border-white/[0.06] bg-[#171717]"}`}>

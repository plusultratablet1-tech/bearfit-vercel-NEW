-- Prefer real package cycles over the historical LEGACY_FITNESS bridge.
-- When a member renews early, continue consuming the oldest active real cycle
-- until it is depleted, then move to the newer active renewal cycle.
...
begin
  ...
  order by
    case
      when d.code <> 'LEGACY_FITNESS' and c.status = 'active' then 0
      when d.code <> 'LEGACY_FITNESS' then 1
      when c.status = 'active' then 2
      else 3
    end,
    case
      when d.code <> 'LEGACY_FITNESS' and c.status = 'active' then c.created_at
    end asc,
    c.created_at desc
  limit 1;
